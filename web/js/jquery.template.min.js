$(function() {
	var $bf_page_menu = $('#bf_page_menu'), $bf_menu_items = $bf_page_menu
			.find('a'), $bf_container = $('#bf_container'), $bf_pages = $bf_container
			.children('.bf_page'), $pageTitle = $bf_page_menu
			.children('.title'), $bf_dishes = $('#bf_dishes > li > a'), $bf_background = $('#bf_background'), $bg_image = $bf_background
			.children('img'), $bf_overlay = $bf_background
			.children('.bf_overlay'), $bf_prev = $('#bf_prev'), $bf_next = $('#bf_next'), $bf_close = $('#bf_close'), $bf_gallery = $('#bf_gallery'), $bf_gallery_wrapper = $bf_gallery
			.children('.bf_gallery_wrapper'), $bf_items = $bf_gallery_wrapper
			.find('.bf_gallery_item'), total_items = $bf_items.length, BGMap = (function() {
		var map, $mapEl = $('#map'),  display = false, 
		showMap = function() {
		...
		var changeBgImageDim = function() {
			...
			var $img = new Image();
			$img.src = $i.attr('src');
			var w_w = $(window).width(), w_h = $(window).height(), r_w = w_h
			...
			return $.Deferred(
					function(dfd) {
						(dir) ? $bf_background.fadeIn(1000, dfd.resolve)
			...
			return $.Deferred(
					function(dfd) {
						...
						$bf_items.each(function(i) {
							var $item = $(this), $itemImg = $item
									.children('img'), itemImgSrc = $itemImg
									.attr('src'), itemBgImgSrc = $itemImg
									.data('bgimg');
		...
		var animated = false, animSpeed = 700, current = 0, defaultImage = 'images/background/default.jpg', init = function(
				position) {
			initEventsHandler();
			slider(position)
		}, stop = function() {
			...
			$bf_close
					.bind(
							...
							function(e) {
								hideNavigation();
								light(500, 0.5);
								var $bf_item_current = $bf_items.eq(current), $heading = $bf_item_current
										.children('.bf_heading'), $desc = $bf_item_current
										.children('.bf_desc');
								animated = true;
								stop();
								$
										.when(
												animateHeadingDesc($heading,
														$desc, 0))
										.done(
												function() {
													$bf_items.css('left',
			...
			var $next_item = $bf_items.eq(current), $headingCurrent = $bf_item_current
					.children('.bf_heading'), $descCurrent = $bf_item_current
					.children('.bf_desc');
			$
					.when(animateHeadingDesc($headingCurrent, $descCurrent, 0))
					.done(
							function() {
								$bf_gallery_wrapper.css('overflow', 'hidden');
								var startingLeft = (dir) ? '480px' : '-520px', currentEndingLeft = (dir) ? '-520px'
										: '480px';
								...
								$next_item
										...
										.animate(
												...
												function() {
													...
													$bf_gallery_wrapper.css(
															'overflow',
															'visible');
													var $heading = $next_item
															.children('.bf_heading'), $desc = $next_item
															.children('.bf_desc');
													$
															.when(
																	animateHeadingDesc(
			...
			var $next_item_bgimage = $('<img src="'
					+ $next_item.children('img').data('bgimg') + '" alt="image'
					+ (current + 1) + '" ></img>'), dim = BGImageController
					.getImageDim($next_item_bgimage), starting_left = (dir) ? $(
					window).width()
					: -dim.width + 'px';
			...
			var $next_bgimage = $('<img src="' + image + '" alt="image'
					+ (current + 1) + '" ></img>'), dim = BGImageController
					.getImageDim($next_bgimage);
			$next_bgimage.css({
				width : dim.width + 'px',
				height : dim.height + 'px',
				left : dim.left,
